...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, storage) {
    ...
    router.on(wire.FileSystemUploadFileMessage, function(channel, message) {
      var reply = wireutil.reply(options.serial)
      var buffer = Buffer.from("") //in case its an empty file
      if (message.file.split(":")[1].length)
        buffer = Buffer.from(message.file.split(",")[1], 'base64')
      var source = new stream.Readable()
      source._read = () => {}
      source.push(buffer)
      source.push(null)
      var fail = false
      adb.push(options.serial, source, message.path)
        .timeout(10000)
        .then(function(upload) {
          return new Promise(function(resolve, reject) {
            upload.on('progress', function(stats) {
              log.info('Pushed %d bytes so far',
        ...
        .catch(function(err) {
          log.warn('Unable to upload file', err.stack)
          push.send([
            channel
          , reply.fail(err.message)
          ])
